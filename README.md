...
|---|---|---|
| `sentence-transformers` (**default**) | Real local model `all-MiniLM-L6-v2` (384-dim). Understands meaning, not just shared words, so *"schedule a meeting"* matches *"create a calendar event"*. | Production / real semantic retrieval. Downloads ~80MB once, then cached. |
...
- **Approximate vector index** (FAISS/hnswlib) for very large tool catalogues. Exact search is the right choice for tens to hundreds of tools.
